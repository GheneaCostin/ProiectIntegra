...
public class MessageService {
    ...
    public List<Message> getConversationHistory(String userId1, String userId2) {
        return messageRepository.findBySenderIdAndReceiverIdOrSenderIdAndReceiverIdOrderByTimestampAsc(userId1, userId2, userId2, userId1);
    }
        ...
        for (Map.Entry<String, Message> entry : latestMessagesMap.entrySet()) {
            ...
            try {
                ...
                if (fullName.equals("Utilizator Necunoscut")) {
                    ...
                    if (userOpt.isPresent()) fullName = userOpt.get().getEmail();
                }
            } catch (Exception e) {}

            conversations.add(new ConversationDTO(partnerId, fullName, lastMsg.getText(), lastMsg.getTimestamp(), lastMsg.isRead()));
        }
